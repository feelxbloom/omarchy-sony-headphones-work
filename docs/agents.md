...
`AGENTS.md` at the repo root is the always-loaded orientation and should stay
short. This file is the reference it points at; the reader-facing wiki is
...
- Run both suites before and after a change: `python3 tests/run.py` (Python)
  ...
  never lints it.
- `make check` validates a clean copy, so it passes with the agent's
  `.opencode/` present.
...
| `AGENTS.md` | Always-loaded agent orientation; points here for detail. |
| `CHANGELOG.md` | Release notes. |
| `Makefile` | The gate and dev helpers: `test`, `lint`, `qmltest`, `check`, `demo`, `verify`, `doctor`. |
| `docs/architecture.md` | The seam-level architecture write-up; folded into [wiki/architecture.md](wiki/architecture.md). |
| `docs/helper-map.md` | The helper symbol index; folded into this file. |
